...
\subsection{$\star$ Was ist Gegenstand der differentiellen Psychologie?}% IMPORTANT
...
\subsection{Was sind Differentialpsychologische Fragestellungen?}
    ...
    \end{itemize}
\subsection{$\star$ Wann entsteht eine Normalverteilung und welche Merkmale folgen dieser zumindestens annährend?} % IMPORTANT
...
\subsection{$\star$ Wann lässt sich auch nach Darwin auf interindividuelle Unterschiede schließen?}  % IMPORTANT
...
\subsection{$\star$ Was ist Persönlichkeit im Sinne von Eysenck und Eysenck 1987?} % IMPORTANT
    ...
    \end{itemize}
\subsection{$\star$ Welche Aufgaben verfolg die nomothetische Methode?} % IMPORTANT
...
\subsection{Was ist ein Konstrukt und welche Arten werden unterschieden?}
    ...
    \end{itemize}
\subsection{$\star$ Was ist eine Eigenschaft (Trait)?}% IMPORTANT
    \begin{itemize}\itemsep-0.5ex
...
\subsection{$\star$ Was trifft auf Typen als Abschnitte in Verteilungsdimensionen zu?}% IMPORTANT
    ...
    \begin{itemize}\itemsep-0.5ex
        \item $\star$ m  Typen als Abschnitte auf Beschreibungsdimensionen % IMPORTANT
...
\subsection{$\star$ Was sind die Vorteile von Selbsteinschätzung?}% IMPORTANT
...
\subsection{$\star$ Was ist der Unterschied zwischen Persönlichkeitstest und Leistungstests?} % IMPORTANT
% Was erfasst Persönlichkeitstest und was erfasst Leistungstests
...
\subsection{$\star$ Was beschreibt man mit einer Korrelation? Was bedeutet negative und positive?} % IMPORTANT
...
\subsection{$\star$ Was ist das Ziel der Explorativen Faktorenanalyse?} % IMPORTANT
Ziel ist die Datenreduktion, Analyse der Struktur von mehreren Variablen
\subsection{$\star$ Was sind die Grundzüge der Zwei-Faktoren-Theorie der Intelligenz?} % IMPORTANT
...
\subsection{$\star$ Was ist der Unterschied zwischen der g-Faktor Theorie und der primär Faktor Theorie?} % IMPORTANT?
???
%% TODO
...
\subsection{$\star$ Was ist fluide und kristalline Intelligenz nach Cattell?} % IMPORTANT
%welche sie mit cattell in verbindung bringen
...
\subsection{$\star$ Guilford, drei Gerenrelle Aspekte um Intelligenz zu beschreiben, welche waren das?} % IMPORTANT
\begin{itemize}
...
\subsection{$\star$ Was sagt das drei Schichten Model von Carroll aus?} % IMPORTANT
...
\subsection{$\star$ Was ist das besondere am Elementary Cognitive Task (ECT)?} % IMPORTANT
% mentale geschwindigkeit zu untersuchen
Sie sind so einfach, dass sie keine interindividuellen\\
Lösungsstrategien zulassen.
\subsection{$\star$ Welchen generellen g-Faktor zur Mentalen Geschwindigkeit hat man beim Sternberg-Paradigma und Inspektionszeit- Paradigma gefunden?} % IMPORTANT
...
\subsection{$\star$ Was ist die Hypothese der neuralen Effizienz?} % IMPORTANT
...
\subsection{$\star$ Gardner hat 3 klassische und 6 weitere Intelligenzen definiert?} % IMPORTANT
% 3 intelligenzen kennen
\begin{itemize}
...
\end{itemize}
\subsection{$\star$ Was verstehen Mayer und Salovey als emotionale Intelligenz?} % IMPORTANT
% zwei schulen, die emotionale Intelligenz, welche eher für Leistungsmerkmal oder xx steht?
The ability...
\begin{itemize}
...
\subsection{$\star$ Gibt es einen bestätigten Zusammenhang zwischen allgemeiner Intelligenz und Arbeitsleistung im Beruf? (Metaanalyse  zitiert, helfen dabei sich sicherer zu fühlen das da was dran ist, gewisse Bestätigung)} % IMPORTANT
...
\subsection{$\star$ Welche Aspekte kennzeichnen den kreativien Prozess? Und was versteht Guilford unter kreativem Denken?} % IMPORTANT
%unterschiedliche stadien kennen können
...
\subsection{$\star$ Was ist divergentes Denken? Und wie unterscheidet sich konvergentes Denken von divergenten? } % IMPORTANT
%TODO
...
\subsection{$\star$ Drei Aspekte des divergentes Denken wiedergeben} % IMPORTANT?
    \begin{labeling}{Problemsensitivität}
...
\subsection{Welche zwei Dimensionen brachte Wundt in die Temperaments-Diskussion ein?}
    ...
    \end{itemize}
\subsection{$\star$ Was versteht Cattell unter Temperament?} % IMPORTANT
    Temperament beschreibt Art und Weiße wie Menschen etwas machen.
\subsection{$\star$ Wieviel Persönlichkeitswesenzüge unterschied in L-Daten und Q-Daten?} % IMPORTANT
%wieviele bei den L daten
%wieviele bei den q daten
...
\subsection{Was wäre für Eysenck eine Eigenschaft und was wäre ein Typ?}
    ...
    \end{labeling}
\subsection{$\star$ Welche Typen hat Eysenck unterschieden? Wie lautet Eysenck Persönlichkeitsmodel?} % IMPORTANT
    ...
    \item Neurotizismus
    \end{itemize}
\subsection{$\star$ Welcher Typ hat die meiste Kritik erhalten und war umstritten?} % IMPORTANT
...
\subsection{$\star$ Welchen Forschungsansatz würden sie am ehesten mit dem Fünf-Faktoren Model in Verbindung bringen?} % IMPORTANT
...
\subsection{$\star$ Welcher besprochene Ansatz der Fünf-Faktoren Modele basiert auf der Analyse von klassischen Fragebögen?} % IMPORTANT 
%Fragebogen verfahren
    NEO Ansatz
\subsection{Welcher Fünf-Faktoren Model ist noch heute aktuell und stark beforscht?}
    Das NEO-PI-R.
\subsection{$\star$ Wie lauten die fünf Hauptdimensionen des NEO Modells?} % IMPORTANT
    \begin{labeling}{N}
...
\subsection{$\star$ In Welcher Dimension unterscheiden sich der Big Five vom NEO Model am deutlichsten?} % IMPORTANT
...
\subsection{$\star$ Namen, der Forscherpersönlichkeit der für das alternative Fünf Faktoren Model steht?} % IMPORTANT
Zuckermann
\subsection{$\star$ Welche Eysencksche Dimension (Faktor) wurde mittels der Arousal Theorie untersucht?} % IMPORTANT
Extraversion
\subsection{$\star$ Was besagt die Arousal Theorie generell?} % IMPORTANT
...
Durch drei Neurontransmitter.
    ...
    \end{labeling}
\subsection{$\star$ Wie lautet die Definition von Sensation Seeking?} % IMPORTANT
    ...
    %zum schutz?
\subsection{$\star$ Was ist Augmenting und Reducing und in welcher Beziehung steht das zu Sensation Seeking?} % IMPORTANT
%beziehung zum senstation seeking
...
\subsection{$\star$ Was ist die Definition von Selbstkonzept?} % IMPORTANT
...
\subsection{$\star$ Welche Art von Elementen kann das Selbstkonzept enthalten?} % IMPORTANT
...
\subsection{$\star$ Wie ist die Selbstwirksamkeitserwartung definiert?} % IMPORTANT
...
\subsection{$\star$ Welches ist der wichtigste Entstehensfaktor von einer hohen Selbstwirksamkeitserwartung?} % IMPORTANT
Handlungsergebnisse in Gestalt eigener Erfolge.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Persönlichkeit V}
\subsection{$\ast$ Welche drei Bereiche untersucht die Positive Psychologie?} %HAUPTFACH
...
\end{labeling}
\subsection{$\ast$ Was sind Charakterstärken im Sinn von Peterson und Seligman?} %HAUPTFACH
...
\subsection{$\ast$ Nennnen Sie drei der in der VIA Klassifikation beschriebenen Charakterstärken.} %HAUPTFACH
...
\end{labeling}
\subsection{$\star$ Welche der Charakterstärken zeigte eine Signifikant Negative Korrelation zur Allgemeinen Selbstwirksamkeitserwartung bei Kinder und Jungendlichen?} % IMPORTANT
%% transportiert eine andere botschaft
%% keine characterstärke die eine negative korrelation aufweist
%% es geht um die botschaft
%% sind definiert um die beitragen um zufrieden, glücklich, deshalb kann es nicht sien das sie im negeativen zusammenhang zusammenliegt
Es gibt keine Charakterstärke die eine negative Korrelation aufweist.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Anlage und Umwelt}
\subsection{$\star$ Durch was wird der Phänotyp bestimmt?} % IMPORTANT
...
\subsection{$\star$ Welche Aspekte fließen in die phänotypische Varianz ein?} % IMPORTANT
...
\subsection{$\star$ Welche Umwelteffekte gibt es?} % IMPORTANT
$V_U \,\to\,$ Umwelteffekte
\begin{labeling}{VC}
...
\subsection{$\star$ Was ist die Logik der Zwilingsforschung?} % IMPORTANT
%zwilingsmethode (kurz)
% zwilingsforschung verstanden haben, 50 prozentiges
\begin{itemize}
...
\subsection{$\star$ Wieviel Prozent der Phänotypischen Varianz ist bei Persönlichkeitsmerkmale bedingt?} % IMPORTANT
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Stabilität}
\subsection{$\star$ Was bedeutet Absolute Stabilität?} % IMPORTANT
...
\subsection{$\star$ Was bedeutet differentielle Stabilität?} % IMPORTANT
...
\end{itemize}
\subsection{$\star$ Absolut betrachtet, wie verläuft die fluide Intelligenz zwischen dem 25 und 80 Lebensjahr?} % IMPORTANT
...
\subsection{$\star$ Differentiel betrachtet, ab welchem Alter konsolidieren sich individuelle Unterschiede in der Intelligenz?} % IMPORTANT 
Ab einem Alter von ca 10 Jahren.
% ab 10 konsolidieren sich rangleistung der intellenz
\subsection{$\star$ Wissen, Absolut betrachtet, Wie ist der Abfall der Big Five, NEO-PI-R?} % IMPORTANT
...
\subsection{$\star$ Differentielle betrachtet, ab welchem Alter kann man davon sprechen das die Persönlichkeit eines Menschen sehr stabil bleibt?} % IMPORTANT
% 50 bei Persönlichkeit als sehr sehr stabil
...
\subsection{$\star$ Welches statistische Maß wird oft benutzt um Unterscheide der Mittelwerte zweier Gruppen zu bestimmen?} % IMPORTANT 
...
\subsection{$\star$ In welchen der fünf Faktoren unterscheiden sich Männer von Frauen und in welche Richtung?} % IMPORTANT
...
\end{labeling}
\subsection{$\star$ Was nennt der Sozial-konstruktivistischer Erklärungsansatz im Bezug in Erklärung von Geschlechterunterschieden an?} % IMPORTANT
