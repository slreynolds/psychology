\documentclass[a6paper,9pt,DIV=14]{scrartcl}
\usepackage{graphicx}
...
\subsection{Was ist Gegenstand der differentiellen Psychologie?}% IMPORTANT
...
\subsection{Was sind Differentialpsychologische Fragestellungen?}
    ...
    \end{itemize}
\subsection{Wann entsteht eine Normalverteilung und welche Merkmale folgen dieser zumindestens annährend?} % IMPORTANT
...
\subsection{Wann lässt sich auch nach Darwin auf interindividuelle Unterschiede schließen?}  % IMPORTANT
...
\subsection{Was ist Persönlichkeit im Sinne von Eysenck und Eysenck 1987?} % IMPORTANT
    ...
    \end{itemize}
\subsection{Welche Aufgaben verfolg die nomothetische Methode?} % IMPORTANT
...
\subsection{Was ist ein Konstrukt und welche Arten werden unterschieden?}
    ...
    \end{itemize}
\subsection{Was ist eine Eigenschaft (Trait)?}% IMPORTANT
    \begin{itemize}\itemsep-0.5ex
...
\subsection{Was trifft auf Typen auf Abschnitte auf Verteilungsdimensionen?}% IMPORTANT
    ...
    \begin{itemize}\itemsep-0.5ex
        \item Typen als Abschnitte auf Beschreibungsdimensionen % IMPORTANT
...
\subsection{Was sind die Vorteile von Selbsteinschätzung?}% IMPORTANT
...
\subsection{Was ist der Unterschied zwischen Persönlichkeitstest und Leistungstests?} % IMPORTANT
...
\subsection{Was beschreibt man mit einer Korrelation? Was bedeutet negative und positive?} % IMPORTANT
...
\subsection{Was ist das Ziel der Explorativen Faktorenanalyse?} % IMPORTANT
Ziel ist die Datenreduktion, Analyse der Struktur von mehreren Variablen
\subsection{Was sind die Grundzüge der Zwei-Faktoren-Theorie der Intelligenz?} % IMPORTANT
...
\subsection{Was ist der Unterschied zwischen der g-Faktor Theorie und der primär Faktor Theorie?} % IMPORTANT?
???
%% TODO
...
\subsection{Was ist fluide und kristalline Intelligenz nach Cattell?} % IMPORTANT
...
\subsection{Guilford, drei Gerenrelle Aspekte um Intelligenz zu beschreiben, welche waren das?} % IMPORTANT
\begin{itemize}
...
\subsection{Was sagt das drei Schichten Model von Carroll aus?} % IMPORTANT
...
\subsection{Was ist das besondere am Elementary Cognitive Task (ECT)?} % IMPORTANT
% mentale geschwindigkeit zu untersuchen
Sie sind so einfach, dass sie keine interindividuellen\\
Lösungsstrategien zulassen.
\subsection{Welchen generellen g-Faktor zur Mentalen Geschwindigkeit hat man beim Sternberg-Paradigma und Inspektionszeit-Paradigma gefunden?} % IMPORTANT
...
\subsection{Was ist die Hypothese der neuralen Effizienz?} % IMPORTANT
...
\subsection{Gardner hat 3 klassische und 6 weitere Intelligenzen definiert?} % IMPORTANT
...
\end{itemize}
\subsection{Was verstehen Mayer und Salovey als emotionale Intelligenz?} % IMPORTANT
...
\subsection{Gibt es einen bestätigten Zusammenhang zwischen allgemeiner Intelligenz und Arbeitsleistung im Beruf? (Metaanalyse zitiert, helfen dabei sich sicherer zu fühlen das da was dran ist, gewisse Bestätigung)} % IMPORTANT
...
\subsection{Welche Aspekte kennzeichnen den kreativien Prozess? Und was versteht Guilford unter kreativem Denken?} % IMPORTANT
...
\subsection{Was ist divergentes Denken? Und wie unterscheidet sich konvergentes Denken von divergenten? } % IMPORTANT
%TODO
...
\subsection{Drei Aspekte des divergentes Denken wiedergeben} % IMPORTANT?
    \begin{labeling}{Problemsensitivität}
...
\subsection{Welche zwei Dimensionen brachte Wundt in die Temperaments-Diskussion ein?}
    ...
    \end{itemize}
\subsection{Was versteht Cattell unter Temperament?} % IMPORTANT
    Temperament beschreibt Art und Weiße wie Menschen etwas machen.
\subsection{Wieviel Persönlichkeitswesenzüge unterschied in L-Daten und Q-Daten?} % IMPORTANT
...
\subsection{Was wäre für Eysenck eine Eigenschaft und was wäre ein Typ?}
    ...
    \end{labeling}
\subsection{Welche Typen hat Eysenck unterschieden? Wie lautet Eysenck Persönlichkeitsmodel?} % IMPORTANT
    ...
    \item Neurotizismus
    \end{itemize}
\subsection{Welcher Typ hat die meiste Kritik erhalten und war umstritten?} % IMPORTANT
...
\subsection{Welchen Forschungsansatz würden sie am ehesten mit dem Fünf-Faktoren Model in Verbindung bringen?} % IMPORTANT
...
\subsection{Welcher besprochene Ansatz der Fünf-Faktoren Modele basiert auf der Analyse von klassischen Fragebögen?} % IMPORTANT
%Fragebogen verfahren
    NEO Ansatz
\subsection{Welcher Fünf-Faktoren Model ist noch heute aktuell und stark beforscht?}
    Das NEO-PI-R.
\subsection{Wie lauten die fünf Hauptdimensionen des NEO Modells?} % IMPORTANT
    \begin{labeling}{N}
...
\subsection{In Welcher Dimension unterscheiden sich der Big Five vom NEO Model am deutlichsten?} % IMPORTANT
...
\subsection{Namen, der Forscherpersönlichkeit der für das alternative Fünf Faktoren Model steht?} % IMPORTANT
Zuckermann
\subsection{Welche Eysencksche Dimension (Faktor) wurde mittels der Arousal Theorie untersucht?} % IMPORTANT
Extraversion
\subsection{Was besagt die Arousal Theorie generell?} % IMPORTANT
...
Durch drei Neurontransmitter.
    ...
    \end{labeling}
\subsection{Wie lautet die Definition von Sensation Seeking?} % IMPORTANT
    ...
    %zum schutz?
\subsection{Was ist Augmenting und Reducing und in welcher Beziehung steht das zu Sensation Seeking?} % IMPORTANT
...
\subsection{Was ist die Definition von Selbstkonzept?} % IMPORTANT
...
\subsection{Welche Art von Elementen kann das Selbstkonzept enthalten?} % IMPORTANT
...
\subsection{Wie ist die Selbstwirksamkeitserwartung definiert?} % IMPORTANT
...
\subsection{Welches ist der wichtigste Entstehensfaktor von einer hohen Selbstwirksamkeitserwartung?} % IMPORTANT
Handlungsergebnisse in Gestalt eigener Erfolge.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Persönlichkeit V}
\subsection{Welche drei Bereiche untersucht die Positive Psychologie?} %HAUPTFACH
...
\end{labeling}
\subsection{Was sind Charakterstärken im Sinn von Peterson und Seligman?} %HAUPTFACH
...
\subsection{Nennnen Sie drei der in der VIA Klassifikation beschriebenen Chrackterstärken.} %HAUPTFACH
...
\end{labeling}
\subsection{Welche der Charakterstärken zeigte eine Signifikant Negative Korrelation zur Allgemeinen Selbstwirksamkeitserwartung bei Kinder und Jungendlichen?} % IMPORTANT
...
\subsection{Durch was wird der Phänotyp bestimmt?} % IMPORTANT
% was wir beobachten können
Phänotyp (Erscheinungsbild), das was man sieht. Wird vom Genotyp (genetische Ausstattung) bestimmt.
\subsection{Welche Aspekte fließen in die phänotypische Varianz ein?} % IMPORTANT
...
\subsection{Welche Umwelteffekte gibt es?} % IMPORTANT
\begin{labeling}{VC}
...
\subsection{Was ist die Logik der Zwilingsforschung?} % IMPORTANT
...
\subsection{Wieviel Prozent der Phänotypischen Varianz ist bei Persönlichkeitsmerkmale bedingt?} % IMPORTANT
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Stabilität}
\subsection{Was bedeutet Absolute Stabilität?} % IMPORTANT
...
\subsection{Was bedeutet differentielle Stabilität?} % IMPORTANT
...
\end{itemize}
\subsection{Absolut betrachtet, wie verläuft die fluide Intelligenz zwischen dem 25 und 80 Lebensjahr?} % IMPORTANT
...
\subsection{Differentiel betrachtet, ab welchem Alter konsolidieren sich individuelle Unterschiede in der Intelligenz?} % IMPORTANT
Ab einem Alter von ca 10 Jahren.
% ab 10 konsolidieren sich rangleistung der intellenz
\subsection{Wissen, Absolut betrachtet, Wie ist der Abfall der Big Five, NEO-PI-R?} % IMPORTANT
...
\subsection{Differentielle betrachtet, ab welchem Alter kann man davon sprechen das die Persönlichkeit eines Menschen sehr stabil bleibt?} % IMPORTANT
...
\subsection{Welches statistische Maß wird oft benutzt um Unterscheide der Mittelwerte zweier Gruppen zu bestimmen?} % IMPORTANT
...
\subsection{In welchen der fünf Faktoren unterscheiden sich Männer von Frauen und in welche Richtung?} % IMPORTANT
...
\end{labeling}
\subsection{Was nennt der Sozial-konstruktivistischer Erklärungsansatz im Bezug in Erklärung von Geschlechterunterschieden an?} % IMPORTANT
